...
if [ ! -d toolchain/ ]; then
  mkdir toolchain/
  echo "$ANDROID_NDK/build/tools/make-standalone-toolchain.sh --arch=$PLATFORM --platform=android-14 --install-dir=./toolchain --ndk-dir=$ANDROID_NDK --system=linux-x86_64 --toolchain=$toolchain"
  $ANDROID_NDK/build/tools/make-standalone-toolchain.sh --arch=$PLATFORM --platform=android-14 --install-dir=./toolchain --ndk-dir=$ANDROID_NDK --system=linux-x86_64 --toolchain=$toolchain
fi
